...
From an integration perspective, Babylon is very different from Olympia. The thing that will stay is the token balances: Babylon will start automatically when Olympia ends, with Olympia’s end state forming Babylon’s genesis. Token balances will be the same, controlled by the same keys, and validator stakes will stay.  
But integrating with Olympia requires a separation integration. Compared with Olympia, in Babylon:
