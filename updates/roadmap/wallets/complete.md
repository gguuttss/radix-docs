...
-   **Smarter transfer flow deposits (eliminate "extra" signatures when possible)**
    -   Check user-controlled deposit accounts for deposit rules, use \`try\_deposit\` if the resource to be deposited would not actually be rejected - use \`deposit\` and surface "signature required" message when it would
