...
These instructions are only available in `V2` manifests, which is supported only by `SubintentManifestV2` and `TransactionManifestV2`. For the time being, the Radix Wallet will only support:

-   Childless `SubintentManifestV2` stubs for pre-authorization requests
