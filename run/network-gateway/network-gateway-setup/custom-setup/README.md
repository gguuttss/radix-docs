...
Before starting with custom setup, read the [Network Gateway architecture and setup overview](../README.md).
...
Depending on your infrastructure provider, there are many ways to deploy these components - so in these docs, we’ll stick to the [recommended requirements for a production setup](requirements.md), and explain how the services can be [configured](configuration.md) and [monitored](../../maintenance-1/monitoring.md) so that this can be adapted to your infrastructure requirements.
...
If [deploying the Grafana / Monitoring stack](../../../node/node-maintenance-and-administration/node-setting-up-grafana.md), we would always recommend this is deployed onto a separate host.